...
# ------------------ DATABASE ------------------
# Updated to match the lowercase name in your Compass screenshots
...
async def login(username: str = Form(...), password: str = Form(...)):
    # Updated to check 'email' field based on your screenshot
...
async def upload_scan(
    ...
    try:
        ...
        # --- FIX: Ensure buffer is at start for Pillow ---
        ...
        try:
            ...
            print(f"Pillow Image Error: {img_err}")
            raise HTTPException(status_code=400, detail="Could not identify image file. Try a standard JPG/PNG.")
        ...
        # -------- Heatmap --------
        max_val = stone_conf_map.max()
        heatmap_norm = stone_conf_map / max_val if max_val > 0 else stone_conf_map

        heatmap_resized = cv2.resize(heatmap_norm, (image.width, image.height))
